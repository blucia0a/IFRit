...
IFR *getWriteIFR(unsigned long varg) {
  IFR *i = g_hash_table_lookup(ACTIVE_MUST_WRITE_TABLE(varg),
			       (gconstpointer) varg);
...
void activateWriteIFR(unsigned long varg, void *PC, unsigned long id){
  ...
  IFR *i = g_hash_table_lookup(ACTIVE_MUST_WRITE_TABLE(varg),(gconstpointer)varg);
  g_hash_table_insert(ACTIVE_MUST_WRITE_TABLE(varg), (gpointer)varg, new_ifr(pthread_self(),id,(unsigned long)PC,varg));
  if (i) {
    delete_ifr(i);
  }
...
void IFR_raceCheck(gpointer key, gpointer value, gpointer data){
  ...
  if (!pthread_equal(ifr->thread, me->thread)) {
    //raceCount++;
    fprintf(stderr,"[IFRit] %lu %lu : %p %p\n", me->id, ifr->id, me->instAddr, ifr->instAddr);
#ifdef RACESTACK
    print_trace();
#endif
  }
